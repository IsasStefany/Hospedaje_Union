...
public class ReservaDAO {
    ...
    public int registrarHuespedYReserva(Huesped huesped, Reserva reserva) {
        ...
        try {
            ...
            if (huespedId == -1) throw new SQLException("No se pudo insertar huesped.");

            reserva.setHuespedId(huespedId);
            int reservaId = insertarReserva(con, reserva);
            if (reservaId == -1) throw new SQLException("No se pudo insertar reserva.");
    ...
    public List<Reserva> listarActivas() {
        ...
        String sql = "SELECT r.id, r.habitacion_id, r.precio_cobrado, r.uso_camas, "
                + "r.fecha_entrada, r.fecha_vencimiento, r.estado, r.metodo_pago, "
        ...
        try (Connection con = Conexion.getConexion();
            ...
            while (rs.next()) {
                ...
                re.setMetodoPago(rs.getString("metodo_pago"));
    ...
    public boolean finalizarReserva(int reservaId, int habitacionId) {
        ...
        try {
            ...
            try (PreparedStatement ps = con.prepareStatement(
                    "UPDATE reservas SET estado='FINALIZADA', fecha_salida_real=GETDATE() WHERE id=?")) {
                ps.setInt(1, reservaId); ps.executeUpdate();
            }
    ...
    public double totalIngresosHoy() {
        String sql = "SELECT ISNULL(SUM(precio_cobrado),0) FROM reservas "
    ...
    private int insertarHuesped(Connection con, Huesped h) throws SQLException {
        ...
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setString(1,h.getNombres()); ps.setString(2,h.getApellidos());
            ps.setString(3,h.getDni());     ps.setString(4,h.getProcedencia());
            ps.setString(5,h.getMotivoVisita()); ps.setString(6,h.getTelefono());
    ...
    private int insertarReserva(Connection con, Reserva r) throws SQLException {
        String sql = "INSERT INTO reservas (huesped_id,habitacion_id,usuario_id,"
                + "fecha_entrada,fecha_vencimiento,precio_cobrado,uso_camas,metodo_pago,estado) "
                + "VALUES (?,?,?,?,?,?,?,?,'ACTIVA'); SELECT SCOPE_IDENTITY();";
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setInt(1,r.getHuespedId());       ps.setInt(2,r.getHabitacionId());
            ...
            ps.setString(8, r.getMetodoPago() != null ? r.getMetodoPago() : "EFECTIVO");
            try (ResultSet rs = ps.executeQuery()) { if (rs.next()) return rs.getInt(1); }
        }
        return -1;
    }

    private void actualizarEstadoHabitacion(Connection con, int habId, String estado) throws SQLException {
        try (PreparedStatement ps = con.prepareStatement(
                "UPDATE habitaciones SET estado=? WHERE id=?")) {
            ps.setString(1,estado); ps.setInt(2,habId); ps.executeUpdate();
        }
    }

    private void actualizarCajaDiaria(Connection con, java.math.BigDecimal monto) throws SQLException {
        String sql = "IF EXISTS (SELECT 1 FROM caja_diaria WHERE fecha=CAST(GETDATE() AS DATE)) "
                + "  UPDATE caja_diaria SET total_ingresos=total_ingresos+?,saldo_neto=saldo_neto+? "
                + "  WHERE fecha=CAST(GETDATE() AS DATE) "
                + "ELSE INSERT INTO caja_diaria (fecha,total_ingresos,saldo_neto) "
                + "  VALUES (CAST(GETDATE() AS DATE),?,?)";
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setBigDecimal(1,monto); ps.setBigDecimal(2,monto);
            ps.setBigDecimal(3,monto); ps.setBigDecimal(4,monto);
            ps.executeUpdate();
        }
    }
}